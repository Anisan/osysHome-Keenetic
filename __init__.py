...
from app.database import convert_utc_to_local, get_now_to_utc, row2dict, session_scope
...
class Keenetic(BasePlugin):
    ...
    @staticmethod
    def _fmt_updated_local(utc_str) -> str:
        try:
            dt = datetime.strptime(str(utc_str or ""), "%Y-%m-%d %H:%M:%S")
            return convert_utc_to_local(dt).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def admin(self, request_):
        ...
        if router_id:
            ...
            for _d in devices:
                _d["updated"] = self._fmt_updated_local(_d.get("updated"))
            for _v in vpns:
                _v["updated"] = self._fmt_updated_local(_v.get("updated"))
        ...
        for _r in routers:
            _r["updated"] = self._fmt_updated_local(_r.get("updated"))
